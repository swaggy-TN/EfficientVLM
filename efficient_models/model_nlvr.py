...
import time
from efficient_models.xvlm import XVLMBase, load_pretrained,build_mlp
from efficient_models.eff_bert import BertConfig
from efficient_models.nlvr_l0_module import NLVRL0Module
...
class EffXVLMForNLVR(XVLMBase):
    def __init__(self, config):
        ...
        self.l0_module = NLVRL0Module(config,target_sparsity=config['sparsity'])
    ...
    def forward(self, image, text_ids, text_atts, targets, train=True,output_attentions=None,output_hidden_states=None):
        ...
        if output_attentions:
            ...
            output_cls = self.get_cross_embeds([image0_embeds, image1_embeds], [image_atts[:image0_embeds.size(0)], image_atts[image0_embeds.size(0):]],
                                                ...
                                                head_z=torch.cat((text_head_z,cross_head_z),dim=0),
                                                mlp_z=torch.cat((text_mlp_z,cross_mlp_z),dim=0))[:, 0, :]
