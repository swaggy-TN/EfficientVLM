...
def build_text_encoder(config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None):
    ...
    if use_mlm_loss:  # for pre-training, load_text_params by default (otherwise notimplemented)
        ...
        if 'bert-base-uncased' in config['text_encoder']:
            pass
        elif 'roberta-base' in config['text_encoder']:
...
class XVLMBase(nn.Module):
    ...
    def get_vision_embeds(self, image, image_atts=None, idx_to_group_img=None,output_attentions=None,output_hidden_states=None):
        """
    ...
    def get_text_embeds(self, text_ids, text_atts,output_attentions=None,output_hidden_states=None):
    ...
    def get_matching_loss(self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat, idx=None,output_attentions=None,output_hidden_states=None):
        """
        ...
        if output_hidden_states:
            ...
            neg_last_hidden,neg_hidden_states,neg_attentions,neg_cross_attentions = self.get_cross_embeds(image_embeds_all, image_atts_all, text_embeds=text_embeds_all,
                                          text_atts=text_atts_all,output_attentions=output_attentions,output_hidden_states=output_hidden_states)
    ...
    def get_mlm_loss(self, text_ids_masked, text_atts, image_embeds, image_atts, masked_pos, masked_ids,output_attentions=None,output_hidden_states=None):
        assert output_hidden_states == output_attentions
        if not output_attentions:
    ...
    def predict_bbox(self, image_embeds, text_embeds, text_atts,output_attentions=None,output_hidden_states=None):
        """
        Args:
        ...
        if output_attentions is not None:
            outputs = self.get_cross_embeds(image_embeds, torch.ones(image_embeds.shape[:2]).to(image_embeds.device),
                                           text_embeds=text_embeds, text_atts=text_atts,output_attentions=output_attentions,output_hidden_states=output_hidden_states)
