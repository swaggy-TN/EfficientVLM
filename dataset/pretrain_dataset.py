...
def build_tokenizer(text_encoder: str):
    if 'bert-base-uncased' in text_encoder:
        tokenizer = BertTokenizer.from_pretrained(text_encoder)

    elif 'roberta-base' in text_encoder:
    ...
    def __init__(self, config, data_path, rank=0, world_size=1, shuffle=True, repeat=True, transform=None, add_eos=False):
        ...
        if 'images' in config.keys():
            ...
            if 'language_chosen' in config['images'].keys():
                assert isinstance(config['images']['language_chosen'], str)
        ...
        print(f"dataset.cls_token: {self.tokenizer.cls_token} ({self.tokenizer.cls_token_id})", flush=True)
        print(f"dataset.eos/sep_token: {self.tokenizer.sep_token} ({self.tokenizer.sep_token_id})", flush=True)
        print(f"dataset.pad_token: {self.tokenizer.pad_token} ({self.tokenizer.pad_token_id})", flush=True)
        print(f"dataset.mask_token: {self.tokenizer.mask_token} ({self.tokenizer.mask_token_id})", flush=True)

        if 'bert-base-uncased' not in config['text_encoder']:
...
class RegionTextJsonDataset(ImageTextJsonDataset):
    def __init__(self, config, data_path, rank=0, world_size=1, shuffle=True, repeat=True, transform=None, box_transform=None):
        ...
        if 'language_chosen' in config['regions'].keys():
            assert isinstance(config['regions']['language_chosen'], str)
            self.language_chosen = config['regions']['language_chosen']
