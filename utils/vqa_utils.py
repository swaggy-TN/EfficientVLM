...
def update_params(model, zs):
    ...
    text_decoder = model.text_decoder.bert

    dims_per_head = 64
    vision_layers,text_layers,cross_layers,decoder_layers = 6,3,3,3
    ...
    if "decoder_intermediate_z" in zs:
        for layer in range(decoder_layers):
            intermediate_z = zs["decoder_intermediate_z"][layer].cpu().squeeze().clone()
            text_decoder.encoder.layer[layer].output.dense.weight.data = text_decoder.encoder.layer[layer].output.dense.weight.data.mul(intermediate_z)
    if "decoder_head_z" in zs:
        for layer in range(decoder_layers):
            i = layer * 2
            self_head_z = zs["decoder_head_z"][i].cpu().squeeze().clone()
            decoder_head_z = zs["decoder_head_z"][i+1].cpu().squeeze().clone()
            self_head_z = torch.repeat_interleave(self_head_z, dims_per_head)
            decoder_head_z = torch.repeat_interleave(decoder_head_z,dims_per_head)

            text_decoder.encoder.layer[layer].attention.self.value.weight.data = text_decoder.encoder.layer[layer].attention.self.value.weight.transpose(0, 1).data.mul(self_head_z).transpose(0, 1)
            text_decoder.encoder.layer[layer].attention.self.value.bias.data = text_decoder.encoder.layer[layer].attention.self.value.bias.data.mul(self_head_z)
            text_decoder.encoder.layer[layer].crossattention.self.value.weight.data = text_decoder.encoder.layer[layer].crossattention.self.value.weight.transpose(0, 1).data.mul(decoder_head_z).transpose(0, 1)
            text_decoder.encoder.layer[layer].crossattention.self.value.bias.data = text_decoder.encoder.layer[layer].crossattention.self.value.bias.data.mul(decoder_head_z)
...
def prune_model_with_z(zs, model):
    ...
    if "decoder_head_z" in zs:
        head_z = zs.get("decoder_head_z", None)
        prune_heads = {}
        for layer in range(len(head_z)):
            head_z_layer = head_z[layer].cpu().squeeze().clone()
            index = torch.where(head_z_layer == 0)[0].tolist()
            prune_heads[layer] = index
            print(f"Layer {layer}, heads {' '.join([str(i) for i in index])} pruned.")
        text_decoder.prune_heads(prune_heads,is_cross='decoder')
    ...
    if "decoder_intermediate_z" in zs:
        kept_intermediate_dims = {}
        intermediate_zs = zs["decoder_intermediate_z"]
        for layer in range(len(intermediate_zs)):
            intermediate_z_layer = intermediate_zs[layer].squeeze()
            intermediate_z_layer = intermediate_z_layer.cpu().clone()
            kept_intermediate_dims[layer] = intermediate_z_layer.nonzero().reshape(-1).tolist()        
    if kept_intermediate_dims is not None:
        prune_intermediate_layers(text_decoder, kept_intermediate_dims,text_encoder.device)
...
#display cross_encoder           
    ...
    for layer in range(decoder_layers):
        print("Layer:", layer)
        if text_decoder.encoder.layer[layer].attention.self.query is not None:
            print("self attention query:", text_decoder.encoder.layer[layer].attention.self.query.weight.shape)
            print("self attnetion key:", text_decoder.encoder.layer[layer].attention.self.key.weight.shape)
        else:
            print("self attention query:", None)
            print("self attnetion key:", None)
        if text_decoder.encoder.layer[layer].crossattention.self.query is not None:
            print("cross attention query:", text_decoder.encoder.layer[layer].crossattention.self.query.weight.shape)
            print("cross attnetion key:", text_decoder.encoder.layer[layer].crossattention.self.key.weight.shape)
        else:
            print("cross attention query:", None)
            print("cross attnetion key:", None)

        if text_decoder.encoder.layer[layer].attention.self.value is not None:
            print("self attention value:", text_decoder.encoder.layer[layer].attention.self.value.weight.shape)
            print("self attention output:", text_decoder.encoder.layer[layer].attention.output.dense.weight.shape)
        else:
            print("self attention value:", None)
            print("self attention output:", None)

        if text_decoder.encoder.layer[layer].crossattention.self.value is not None:
            print("cross attention value:", text_decoder.encoder.layer[layer].crossattention.self.value.weight.shape)
            print("cross attention output:", text_decoder.encoder.layer[layer].crossattention.output.dense.weight.shape)
        else:
            print("cross attention value:", None)
            print("cross attention output:", None)


        if text_decoder.encoder.layer[layer].intermediate.dense is not None:
            print("up:", text_decoder.encoder.layer[layer].intermediate.dense.weight.shape)
            print("down:", text_decoder.encoder.layer[layer].output.dense.weight.shape)
        else:
            print("up", None)
            print("down", None)
