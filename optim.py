...
def create_L0_optimizer(args,l0_module):

    l0_params = [{
        "params": [p for n, p in l0_module.named_parameters() if "lambda" not in n],
        "weight_decay": 0.0,
        "lr": args.reg_learning_rate
    }]
    
    l0_optimizer = AdamW(l0_params,eps=1e-8, betas=(0.9, 0.98))
    ...
    lagrangian_optimizer = AdamW(lagrangian_params,eps=1e-8, betas=(0.9, 0.98))
